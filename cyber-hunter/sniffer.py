...
class Sniffer:
    ...
    def _sniffer_thread(self):
        sniff(iface=self.interfaces, prn = self._packet_handler,store=0,quiet=True,stop_filter=lambda x: self.stop_event.is_set())
    ...
    def _packet_handler(self,packet):
        if packet.haslayer(Dot11):
            ...
            if packet.type == 0 and packet.subtype == 8 and packet.haslayer(Dot11Beacon):
                ...
                if not "channel" in beacon_info:
                    beacon_info['channel'] = self._freq_to_channel(packet[RadioTap].Channel)
                beacon_ap_bssid = packet.addr3.replace(":","")
                if not beacon_ap_bssid in self.known_aps:
                    ap_vendor = self._get_mac_vendor(beacon_ap_bssid)
                    logger.info("Discovered new AP(%s) by vendor %s broadcasting \"%s\" on channel %s",str(beacon_ap_bssid),str(ap_vendor),str(beacon_info['ssid']),str(beacon_info['channel']))
                    pmf_config = self._parse_pmf_config(packet)
                    # Add AP to database, and note rowId
                    row_id = self.db_interface.add_ap(beacon_ap_bssid,beacon_info['ssid'],ap_vendor,beacon_info['channel'],', '.join(beacon_info['crypto']),pmf_config)
            ...
            if packet.type == 2 and not packet.haslayer(EAPOL):
                ...
                if not to_ds == from_ds:
                    ...
                    if not client in self.known_clients and self._is_unicast(client):
                        vendor = self._get_mac_vendor(client)
                        logger.info("Discovered new client(%s) by vendor %s connected to %s",str(client),str(vendor),str(bssid))
            ...
            if packet.type == 1 and (packet.subtype == 9 or packet.subtype == 11):
                ...
                if addr1 in self.known_aps and addr2 not in self.known_clients:
                    ...
                    logger.info("Discovered new client(%s) by vendor %s connected to %s",str(addr2),str(vendor),str(addr1))
                ...
                if addr2 in self.known_aps and addr1 not in self.known_clients:
                    ...
                    logger.info("Discovered new client(%s) by vendor %s connected to %s",str(addr1),str(vendor),str(addr2))
                    # Add client to database, and note rowId
    ...
    def _is_locally_administered(self,addr):
        msb = int(addr[0:2], 16)
        return (msb >> 1) & 1
    ...
    def _parse_pmf_config(self,beacon):
