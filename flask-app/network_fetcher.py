...
class NetworkFetcher:
    ...
    @staticmethod
    def _get_vendor(vendor, device_type):
        match vendor:
            case "Apple, Inc.":
    ...
    @staticmethod
    def _get_ssid_nodes(c, row_filter="%"):
    ...
    def _get_ap_nodes(self, c, row_filter="%"):
        ...
        for row in c.fetchall():
            ...
            title = row['bssid'] + "\n" + row['vendor'] + "\nChannel:" + str(row['channel']) + "\n" + row[
                'security'] + "\nPMF:" + pmf_state
            nodes.append(dict(id=row['bssid'], title=title, group="ap", shape="circularImage",
                              image=self._get_vendor(row['vendor'], "AP")))
        return nodes

    def _get_client_nodes(self, c, row_filter="%"):
        """ Returns all Clients as nodes """
        c.execute("SELECT macaddr,vendor FROM Clients WHERE id LIKE ""%s""", (str(row_filter),))
        nodes = [
            dict(id=row['macaddr'], title=row['macaddr'] + "\n" + row['vendor'], group="client", shape="circularImage",
                 image=self._get_vendor(row['vendor'], "Client")) for row in c.fetchall()]
        return nodes

    @staticmethod
    def _get_ap_ssid_edge(c, row_filter="%"):
        """ Returns all edges between APs and SSIDs """
        c.execute("SELECT bssid,ssid FROM APs WHERE id LIKE ""%s""", (str(row_filter),))
        edges = [dict([("from", row['bssid']), ("to", row['ssid'])]) for row in c.fetchall()]
        return edges

    @staticmethod
    def _get_client_ap_edge(c, row_filter="%"):
        """ Returns all edges between clients and APs """
        c.execute("SELECT macaddr,bssid FROM Clients WHERE id LIKE ""%s""", (str(row_filter),))
    ...
    def get_new_data(self, table, row):
        ...
        if table == "APs":
            ...
            cur.execute("SELECT COUNT(*),ssid FROM APs WHERE ssid=(SELECT ssid FROM APs WHERE id=%s) AND id<=%s",
                        (str(row), str(row),))
